"""Per-user canonical file passages and independent file references."""
...
class FileSegment(Base):
    """One exact file passage reused only inside a single user's graph."""
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_file_segment_content_length",
        ),
        UniqueConstraint(
            "user_id",
            "content_sha256",
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("assistant_core.user_identity.id"), nullable=False
    )
...
class FileReference(Base):
    """One owner-scoped file/chunk reference to canonical file text."""
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_file_reference_file_id_length",
        ),
        UniqueConstraint(
            "user_id",
            "native_file_id",
    ...
    segment_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("assistant_core.file_segment.id"), nullable=False
    )
