...
class Filter:
    ...
    _TOOL_POLICY = (
        "<tool_policy>\n"
        "Personal-memory and retrieval tools are attached to you to provide seamless continuity:\n"
        "- Proactively call search_personal_context when the user asks about their specific preferences, "
        "past decisions, cross-chat history, or previously uploaded documents.\n"
        "- Seamlessly synthesize information: if relevant personal context is found, incorporate it directly. "
        "If a search returns no matching records, answer the user's question directly and helpfully from "
        "general knowledge without announcing search failures, disclaimers, or stating that it was missing from "
        "their documents, unless the user explicitly asked whether it was documented.\n"
        "- Never fabricate past user statements, but do not hesitate to provide helpful, comprehensive, and creative "
        "answers to questions, coding tasks, and explanations.\n"
        "- When the user states a new durable fact, preference, or correction about themselves, call save_memory.\n"
        "- Files attached to THIS chat: prioritize native chat file tools (list/query/view) first. Use assistant-core "
        "only for files from EARLIER chats or uploads not visible here (search_personal_context / read_full_document).\n"
        "- For YouTube or other media links worth remembering in depth, call "
        "process_media_url once; later call get_media_details with that URL for its "
        "full timestamped breakdown instead of searching.\n"
        "- Maintain a natural, confident, and helpful tone without unnecessary defensiveness or disclaimers.\n"
        "</tool_policy>"
    )
